...
OUTDIR   = "outputs_learning_june_july"

# Two experimental blocks, analysed SEPARATELY (June = original single sound
# set; July = new-sound / later cohort era). Learning is tracked within each
# block by fitting a per-animal slope of pupil response vs session order.
BLOCKS = {
    "june": ["260603", "260604", "260605", "260606", "260607"],
    "july": ["260717", "260720", "260721", "260722", "260723", "260724", "260725"],
}
...
def load_long(block_dates):
    ...
    long = long[long["date"].isin(block_dates)].reset_index(drop=True)
    if len(long):
...
# ----------------------------------------------------------------------------
def plot_per_animal(df, metric, ylabel, fname, lines, block=""):
    ...
    fig.suptitle(f"{ylabel} across {block.upper()}-block sessions\n"
...
def plot_diff_slopes(df, lines, block=""):
    ...
    fig.suptitle(""
                 f"({block.upper()} block; upward slope = learning)",
                 fontsize=11)
    fig.tight_layout(rect=[0, 0, 1, 0.9])
    f = os.path.join(OUTDIR, f"L2_deviant_minus_normal_slope_{block}.png")
    fig.savefig(f, dpi=150); plt.close(fig); print("saved", f)
...
def mixed_model(df, metric, lab, lines):
    ...
    try:
        ...
        lines.append(f"      mixed model failed ({e}); OLS fallback (no random effect):")
        try:
            mf = smf.ols(f"{metric} ~ sidx * condition", d).fit()
            for k in mf.params.index:
                star = "  <-- learning term" if "sidx:" in k else ""
                lines.append(f"      {k:32s} beta={mf.params[k]:+.4f} "
                             f"p={mf.pvalues[k]:.4f}{star}")
        except Exception as e2:
            lines.append(f"      OLS fallback also failed ({e2})")


def analyse_block(block, dates, all_lines):
    lines = ["=" * 74,
             f"LEARNING ANALYSIS — {block.upper()} BLOCK",
             "=" * 74,
             f"Dates: {dates}",
             "Per-animal slope of pupil response vs session order; a rising",
             "deviant (or deviant-normal) line = across-session learning."]
    long = load_long(dates)
    if long.empty:
        lines.append("  no sessions in this block.")
        all_lines.extend(lines)
        return
    taxis = time_axis(long)
    df = build_table(long, taxis)
    df.to_csv(os.path.join(OUTDIR, f"learning_session_values_{block}.csv"),
              index=False)
    lines.append("\nSessions per animal: "
                 + df.groupby("animal")["sidx"].max().add(1).to_dict().__repr__())

    plot_per_animal(df, "max_resp", "max pupil (0.5-2.0 s)",
                    f"L1_learning_slopes_max_{block}.png", lines, block=block)
    plot_per_animal(df, "mean_resp", "mean pupil (0.5-2.0 s)",
                    f"L1_learning_slopes_mean_{block}.png", lines, block=block)
    plot_diff_slopes(df, lines, block=block)
...
def main():
    ...
    for block, dates in BLOCKS.items():
        print(f"\n########## {block.upper()} BLOCK ##########")
        analyse_block(block, dates, all_lines)

    all_lines.append("=" * 74)
    all_lines.append("How to read this:")
    all_lines.append(" - June and July are analysed as SEPARATE blocks (different")
    all_lines.append("   sound sets / cohorts); slopes are not compared across blocks.")
    all_lines.append(" - Per-animal slopes rest on only a few sessions each, so single")
    all_lines.append("   p-values are weak; the mixed-model sidx:deviant term pools")
    all_lines.append("   animals and is the main learning test within a block.")
    all_lines.append(" - Positive sidx:deviant slope, p<.05 => deviant response grows")
    all_lines.append("   across sessions = evidence of learning in that block.")
