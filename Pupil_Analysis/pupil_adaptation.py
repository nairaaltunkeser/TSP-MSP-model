#!/usr/bin/env python3
# ============================================================================
#  ADAPTATION — early vs late presentations (thesis Figure-13 style)
#  UPDATED for the expanded dataset (EC_opto_pupil2.h5): 5 animals, 14 dates.
#  *** TEST PHASE ONLY *** (trial >= 101).
#
#  Method (matches thesis Fig 13):
#    EARLY = first N trials of the test phase, PER CONDITION, PER SESSION
#    LATE  = last  N trials, same basis.   Thesis uses N = 7.
#    Done for BOTH the normal and the deviant pattern (their panels A and B).
#    SEM is taken across sessions.
#    A session needs >= 2N trials of a condition for a clean non-overlapping
#    split; sparser sessions are plotted but flagged and dropped from that
#    condition's statistics (overlapping trials are labelled 'mid' and unused).
#
#  Also provides a MEDIAN-SPLIT variant (first half vs second half of the
#  session's trials for that condition), which keeps more sessions when the
#  deviant count is low. Set SPLIT_MODE = "median" to use it.
#
#  GROUPING RULES — identical to pupil_analysis_main.py:
#    * NA04 / NA05 -> always 'opto'
...
#               otherwise                          -> 'opto'
#      NA03 exception: 260606/260607 are 'opto' (NA03 only).
#      Note the merged control spans two sound sets (see main script header).
#  RUN DEDUP: keep the HIGHEST run number per animal+date.
#
#  Outputs:
#    A0  early/late count table (csv)
#    A1  per-animal per-session early-vs-late traces (deviant and normal)
#    A2a thesis Fig-13 replica: A = normal, B = deviant, pooled over sessions
#    A2b same split by group (supplementary)
#    A3  late-minus-early difference with cluster-permutation shading
#    A4  per-session adaptation index (late - early max pupil)
#    stats: paired early-vs-late (overall + per group), Kruskal across groups,
#           trial-level mixed model max_resp ~ half + (1|animal) + session vc
# ============================================================================
...
def load_long():
    ...
    # ---- early / late labelling, PER CONDITION PER SESSION ----
    long["half"] = "mid"
...
# control_newsound deviant differs at B, C AND D; others only at C.
...
# ---- A1: per-animal per-session traces -------------------------------------
...
# ---- A2a: thesis Fig-13 replica (pooled) -----------------------------------
def plot_thesis_replica(shm, taxis):
    ...
    fig.suptitle(f"Thesis Fig-13 replica — early vs late ({SPLIT_MODE} split, "
...
# ---- A2b: split by group (supplementary) -----------------------------------
