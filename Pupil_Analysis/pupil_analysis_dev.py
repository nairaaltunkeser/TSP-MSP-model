#!/usr/bin/env python3
# ============================================================================
#  Pupil responses to NORMAL (ABCD) vs DEVIANT (ABAD / deviant_C) sequences
#  across three session types: control | opto | opto_late
#
#  Data model
#  ----------
#  EC_opto_pupil.h5 holds two pandas frame_tables:
#     /normal      -> pupil epochs for standard  ABCD pattern trials
#     /deviant_C   -> pupil epochs for deviant    ABAD pattern trials (C-tone swapped)
#  Each row is ONE trial. The 400 columns are time (s) relative to pattern
#  onset, from -1.00 to +2.99 s at 10 ms steps (100 Hz).
#  Row index carries (time, trial, sess), where sess = "<animal>_<date>_<run>",
#  e.g. NA01_260603_001  ->  animal NA01, date 260603, run 001.
#
#  Pre-processing already applied upstream (see thesis methods): traces are
#  z-scored and BASELINE-CORRECTED (mean of the -1..0 s window subtracted).
#  Verified: baseline-window mean per trial ~ 0, so we do NOT re-correct.
#
#  Stimulus timing (thesis: each pip = 250 ms, 150 ms on + 100 ms off):
#     A : 0.00 - 0.25 s      (5920 Hz    both normal & deviant)
#     B : 0.25 - 0.50 s      (10547 Hz   both)
#     C : 0.50 - 0.75 s      normal = 14079 Hz ; deviant = 5920 Hz (== A)  <-- the deviant
#     D : 0.75 - 1.00 s      (18794 Hz   both)
#  So the informative divergence is expected to emerge from ~C onset (0.5 s)
#  onward, and the analysis window below reflects that.
#
#  Session-type assignment (per user instruction):
#     control    : dates 260606, 260607
#     opto_late  : dates 260618, 260619, 260620
#     opto       : all remaining pattern dates (260603, 260604, 260605)
#  SPECIAL CASES (per user instruction):
#     * For NA03 only, 260606 and 260607 are OPTO (not control).
#     * If an animal+date has both a _000 and _001 run, keep _001, drop _000.
#       (Only NA03_260606 is affected here -> keep NA03_260606_001.)
# ============================================================================
...
# NA04/NA05 later cohort: light-on (opto) vs lights-off (control) days
...
# ABCD pip windows (s), relative to pattern onset at t = 0
PIP_WINDOWS = {
...
# Main "response window" used for scalar summaries (delta / max pupil).
# Chosen to capture the sustained dilation after the pattern; deviance-driven
# divergence is expected from C-onset. We use 0.5 - 2.0 s post-onset.
...
def classify_session(animal, date):
    # NA04/NA05: light-on days = opto, lights-off days = control
    if animal in NA45_ANIMALS:
        if date in NA45_CONTROL_DATES:
            return "control"
        return "opto"
    # NA03 only: 260606/260607 are opto, not control
...
def load_long():
    ...
    # keep only the chosen run per animal+date (001 over 000)
...
def shade_pips(ax, ymin=None, ymax=None):
    ...
    # pip letters just inside the top of the axes
    y0, y1 = ax.get_ylim()
    ytxt = y1 - 0.06 * (y1 - y0)
...
# ============================================================================
# PLOT 1  ---  Per-animal, per-session trace: normal vs deviant
#              (grand mean +/- SEM across trials within that session)
# ============================================================================
def plot_per_session_traces(long, taxis):
    ...
    # one figure per animal; sessions as columns
...
# ============================================================================
# PLOT 2  ---  Grand-average traces by session_type x condition
#              (mean across sessions of the per-session means -> session is
#               the unit, so no pooling bias)
# ============================================================================
...
# ============================================================================
# PLOT 3  ---  Deviant MINUS normal difference trace, per session type
#              with cluster-style pointwise test (paired across sessions)
# ============================================================================
def plot_difference_traces(sess_df, taxis):
    fig, ax = plt.subplots(figsize=(8.5, 4.2))
    summary = []
    for stype in SESSION_TYPE_ORDER:
        # pair normal & deviant within the SAME session
        piv = {}
        for cond in ["normal", "deviant"]:
        ...
        # pointwise one-sample t vs 0 across sessions -> mark p<.05 samples
        if len(shared) >= 3:
...
# ============================================================================
# PLOT 4  ---  Scalar summaries: max/mean pupil in the response window
#              Points = per-session values; box = distribution.
# ============================================================================
...
# ============================================================================
# PLOT 5  ---  Delta pupil (deviant - normal) per session, grouped by type,
#              with shuffle null band (thesis-style).
# ============================================================================
def compute_delta_and_shuffle(long, taxis):
    ...
    for sess, sub in long.groupby("sess"):
        ...
        # shuffle: pool, relabel preserving n per condition
...
def plot_delta(delta_df):
    ...
    for stype in SESSION_TYPE_ORDER:
        ...
        for _, r in sub.iterrows():
            pos += 1
            # 95% shuffle null band drawn as a gray vertical bar
            ax.plot([pos, pos], [r.null_lo, r.null_hi], color="lightgray",
                    lw=6, solid_capstyle="round", zorder=1)
            # observed delta as a colored point on top
...
def run_statistics(sess_df, delta_df):
    ...
    # ---- (1) Within each session type: deviant vs normal (paired across sessions)
    ...
    # ---- (2) Across session types: does the deviant effect differ?
    ...
    # ---- (3) Pairwise across types (Mann-Whitney on Δ)
    ...
    # ---- (4) Per-session shuffle test recap
...
def main():
    ...
    # session-type / trial counts sanity table
