#!/usr/bin/env python3
# ============================================================================
#  PER-SESSION MEAN PUPIL — normal vs deviant, CONTROL and OPTO only
#
#  One panel per session, each showing that session's mean pupil response to
#  the NORMAL sequence vs the DEVIANT sequence (mean +/- SEM across trials).
#  Sessions are laid out in a grid, arranged by group so all CONTROL sessions
#  come first, then all OPTO sessions. This lets you scan session by session
#  for a normal-vs-deviant difference without pooling anything.
#
#  Test phase only (trial >= 101). Grouping + dedup identical to the main
#  analysis (NA04/NA05 = opto; NA03 260606/607 = opto; keep highest run).
#  Only 'control' and 'opto' sessions are shown.
#
#  Output -> outputs_per_session_traces/
#     S1_per_session_normal_vs_deviant.png
# ============================================================================
...
def main():
    ...
    # session order: control block first, then opto, sorted by animal/date
