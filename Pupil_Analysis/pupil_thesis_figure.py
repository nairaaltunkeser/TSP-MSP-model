#!/usr/bin/env python3
# ============================================================================
#  THESIS FIGURE — opto suppresses the deviant surprise response
#
#  Two panels, the visual companion to the control-vs-opto statistics:
#
#   PANEL A  Grouped bars: mean pupil for normal vs deviant, in control vs opto.
#            Bars = group mean +/- SEM (across sessions); dots = individual
#            sessions (paired normal/deviant lines within group). Within-group
#            paired Wilcoxon significance shown with a bracket over each group.
#
#   PANEL B  The interaction, shown directly: per-session deviant-normal
#            difference for each group, with group mean +/- SEM. This is the
#            quantity the condition x group interaction tests. The interaction
#            p-value (from the mixed model) is annotated between the groups.
#
#  Measure: MEAN pupil in the 0.5-2.0 s response window. Test phase only.
#  control + opto only. Grouping + dedup identical to the main analysis.
#
#  Stats recomputed inside so the figure is self-contained:
#    - within-group paired Wilcoxon (deviant vs normal)
#    - between-group Mann-Whitney U on the difference
#    - mixed model mean_resp ~ condition * group + (1|animal): the
#      condition:group interaction is the key test.
#
#  Output -> outputs_thesis_figure/
#     FIG_opto_deviant_interaction.png / .pdf
#     thesis_figure_stats.txt
# ============================================================================
...
def main():
    ...
    # per-session x condition mean-in-window
    rows = []
    ...
    # ---------- PANEL A: grouped bars normal vs deviant, per group ----------
    ...
    for g in KEEP_GROUPS:
        ...
        # paired session dots + connecting lines
        s = wide[wide.group == g]
        ...
        axA.scatter(xd, s["deviant"].values, s=22, color=COND_COLORS["deviant"],
                    edgecolor="k", linewidth=0.4, zorder=5)
        # within-group significance bracket
    ...
    # headroom so brackets clear the dots
    yl = axA.get_ylim()
    ...
    # ---------- PANEL B: the interaction (deviant - normal per group) -------
    ...
    # interaction bracket between the two group bars
    ymax = wide["diff"].max()
    ybr = ymax + 0.12
