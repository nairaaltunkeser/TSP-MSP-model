...
class HipCatModel(nn.Module):
    ...
    def __init__(
        ...
        predictive_ca3: bool  = False,         # False = auto-associative CA3->CA3 (hip-cat); True = heteroassociative
        mossy_recall_mult: float = 0.125,     
        rec_recall_mult:   float = 2.0,        
        ecout_ca1_recall_mult: float = 0.25,  
        theta_gates_ca3: bool | None = None,   
        
        bigloop_from_prev: bool = True,
      
        bigloop_gain:   float = 0.8,
        ...
        self.ecin_to_ca1  = FullConn(input_size, n_ca1, lr_ecin_ca1, hebb_msp,
        ...
        self.ecin_to_dg  = SparseConn(input_size, n_dg,  p_ecin_dg,  lr_ecin_dg,  hebb_tsp,
                                      savg_cor=tsp_cor, seed=sd(1), **sig)
        self.ecin_to_ca3 = SparseConn(input_size, n_ca3, p_ecin_ca3, lr_ecin_ca3, hebb_tsp,
        ...
        self.ca3_to_ca3  = FullConn(n_ca3, n_ca3, lr_ca3_ca3, hebb_tsp,
        ...
        self.ecout_to_ecin = FixedConn(input_size, input_size, p_con=1.0,
        ...
        N = input_size
        for conn, k_s, n_s in [
            (self.ecin_to_ca1,  self.k_ecin,  N),
    ...
    def make_ecin_input(self, cur_idx: torch.Tensor, prev_idx: torch.Tensor):
        """
        Two-hot Input clamp: current=1.0, previous=prev_activity.
        SILENT current -> empty current slot (filled by the big loop in
        ecin_activation). SILENT previous -> the previous slot is taken from
        the one-step delay line of EC_in's current slot (which, after a gap,
        holds the item the network recalled there).
        """
        B = cur_idx.size(0)
        x = torch.zeros(B, self.input_size, device=self.device)
        idx_b = torch.arange(B, device=self.device)
        prev_idx = prev_idx.long()
        cur_idx  = cur_idx.long()
        ok_p = prev_idx >= 0
        ok_c = cur_idx  >= 0
        if ok_p.any():
            x[idx_b[ok_p], prev_idx[ok_p]] = self.prev_activity
        if (~ok_p).any() and self._prev_cur_slot is not None \
                and self._prev_cur_slot.size(0) == B:
            x[~ok_p] = torch.maximum(x[~ok_p],
        ...
        for cyc in range(1, n_cycles + 1):
            ...
            if self.path_on["ecin_ca3"]:
                ca3_contribs.append((self.ecin_to_ca3(ecin),
                                     self.ecin_to_ca3.wt_abs, self.ecin_to_ca3.wt_rel))
    ...
    def eval_step(
        ...
        DG_r, CA3_r, CA1_r, ECout_r, snap_r = self._settle(
            ...
            theta=self.THETA_RECALL, capture_all=capture_all,
