"""
model.py  –  HipCat hippocampal model, PyTorch port of hip-cat.proj (emergent 7.0.1)
             with a working sequence-completion mechanism (v5).

Why the recurrence did not work before (v2)
===========================================
1. kWTA was scale-free (max-normalised). Four DG units fired at full strength
   for ANY cue, so the rel=8 mossy input always dictated CA3 and the rel=1
   recurrent collaterals never had a say, whatever they had learned.
2. Projections were not normalised for size: an 80x80 recurrent projection
   with 5 active senders was compared to a 25 %-sparse perforant path with
   ~0.5 active senders on raw dot products.
3. Raw clipped weights: a learned association (w~0.75) was only 1.5x a random
   one (w~0.5). Leabra's wt_sig (gain 6) turns 0.75 into ~1.0 and 0.25 into
   ~0.0, which is what makes learned attractors dominate novel input.
4. States were zeroed every pip and the big loop ran WITHIN a pip, feeding the
   prediction of the NEXT item back into EC_in while the current item was
   being processed. Nothing about the previous pip reached CA3.
5. Nothing carried the network's own prediction into the next tone, so an
   empty slot stayed empty.

What v5 does
============
  [proj]     read from hip-cat__1_.proj       [paradigm] forced by ABCD-in-noise
  [approx]   Leabra mechanism simplified      [v5]       added for completion

  Layers   : Input/EC_in/EC_out N units; DG 400 (k=4), CA3 80 (k=5), CA1 100 (k=25) [proj]
  Trial    : AUTO minus (30 cyc, EC_in->CA1 on, CA3->CA1 off)  -> act_mid
             RECALL minus (50 cyc, EC_in->CA1 off, CA3->CA1 on, CA1 decayed) -> act_m
             PLUS (20 cyc, both on, EC_out clamped to NEXT item) -> act_p   [proj]
  Learning : Leabra CHL, soft-bounded error + CPCA Hebb w/ savg_cor, wt_sig(6,1);
             MSP contrasts act_mid vs act_p, TSP contrasts act_m vs act_p    [proj]
  Net input: abs*rel/sum(rel) * 1/(expected active inputs) * sum w_eff x    [proj, approx]
  Inhibition: v2 max-normalised threshold-kWTA (act_gain=None). Optional drive-aware variant:
             act_i = min(g_i / max(g), act_gain * g_i),  g = relu(net - thr)  [approx, v5]
             (identical to v2's kWTA when the layer is well driven; a weakly
             driven layer stays weak, so silent/novel input does not fire DG at
             full strength)
  CA3->CA3 : auto-associative, as in hip-cat (drive recomputed every cycle
             from the evolving CA3; learns same-trial co-activity). A
             heteroassociative variant (previous tone's CA3 drives this one)
             is available with predictive_ca3=True.                   [proj / v5 option]
  Mossy    : rel 8 in AUTO and PLUS (encoding), rel 8*mossy_recall_mult in
             RECALL (default 0.125 -> rel 1): retrieval-phase suppression of
             the feed-forward detonator input (Hasselmo & Schnell 1994; Hasselmo
             et al. 2002). Not in hip-cat; without it recurrence loses ~7:1 to
             any DG activity.                                                 [v5]
  Input    : slot_code="separate" (v6 default): EC_in = 2N units, the current
             tone on the first N, the previous tone on the second N. The
             previous tone is thereby carried across the boundary in its own
             units. "previous = B" occurs in one stored pattern only, [C,B], so
             at an omission the partial cue selects [C,B] and auto-associative
             CA3->CA3 completes it - pattern completion with nothing else
             involved. slot_code="shared" is the dissertation's two-hot code
             on N units (then only the big loop can fill an omission).   [v6]
  Big loop : EC_out -> EC_in (into the current-slot units) carries the previous
             pip's prediction into an empty slot; gain bigloop_gain, which is
             0.0 (inert) under "separate" and 0.8 under "shared".      [proj, v5]
  Delay line: the "previous" slot of the window is the previous pip's current
             slot; after a SILENT pip it holds the recalled item.             [v5]
  Readout  : prediction from the AUTO (MSP) phase (eval_readout="auto"); layer
             states for RSMs from the end of RECALL (snapshot_phase="recall",
             hip-cat act_m). The CA3-driven readout of [B,A] is bistable
             (predicts B for several epochs, then C) because CA3([B,A]) shares
             perforant-path units with the variable CA3([A,bg]) codes.        [v5]
  Updates  : summed over the B streams (B hip-cat trials), scaled by
             effective_trials/B (8/B): the plain sum overshoots and flips the
             readout between epochs.                                          [v5]

Gaps
====
  SILENT (-1) as an item index = empty clamp in that slot (true omission).
  Verified (CPU, B=32, T=200, 10 epochs, seed 0):
    prediction (MSP readout)  A->B 0.94  B->C 1.00  C->D 1.00  background at chance
    omitted C: D predicted at the gap 1.00; big loop off 0.00; CA3->CA3 off 1.00
    CA3 overlap with the standard code: gap 1.00, following D 1.00 (CA1 0.96/1.00)
    AB deviant: B* 0.09, C 0.32, D 1.00;  D deviant: D* 0.25  (CA3)
  A substituted item (deviants.py "gap": A B G D) is NOT completed by this
  model: the slot is occupied, the big loop cannot write into it, and an
  auto-associative CA3 encodes the novel [G,B] cue as a new pattern. Only the
  heteroassociative option pushes against present input there.

Switches (all in __init__): predictive_ca3, mossy_recall_mult, rec_recall_mult,
ecout_ca1_recall_mult, effective_trials, act_gain, wt_sig_gain, act_scale,
bigloop_from_prev, bigloop_gain, state_carry (= 1 - decay.event), eval_readout,
snapshot_phase.
"""
