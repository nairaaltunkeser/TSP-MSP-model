...
def fig_seq_sound_decode(model, figdir, dev_types, batch, T, seed):
    ...
    # A silent (omitted) pip has no true sound: its label is SILENT (-1), not
    # one of the 16 classes, so scoring it against -1 gives 0.00 by
    # construction and says nothing about whether the layer filled the gap in.
    # Instead: keep those pips out of the decoder's training set and score them
    # against the OMITTED item (C).
    silent_gap = np.zeros(len(y_all), dtype=bool)
    if "silent" in seq_types:
        si_sil = seq_types.index("silent")
        silent_gap = (tags[:, 0] == si_sil) & (tags[:, 1] == 2)
    y_score = y_all.copy()
    y_score[silent_gap] = WORD[2]          # the omitted C

    # One shared train/test split across the pips that carry a real sound,
    # stratified by sound so every class is represented in train.
    real = idx_all[y_all >= 0]
    tr, te = train_test_split(real, test_size=0.35, random_state=0,
                              stratify=y_all[real])
    te = np.concatenate([te, idx_all[silent_gap]])   # gap pips: test only
    ...
    for st in seq_types:
        if st in ("gap", "silent"):
            pos_headers_by_type[st] = (["A", "B", "gap->C", "D"] if st == "silent"
                                       else ["A", "B", "gap", "D"])
        elif st == "standard":
    ...
    fig.suptitle("Per-pip sound decoding for each sequence  (train pooled, test held-out)\n"
                 "cell = P(decode true sound at that pip); for an omitted pip, "
                 "P(decode the missing C).  * marks the deviating pip.",
                 **STYLE)
        ...
        for ri, st in enumerate(seq_types):
            ...
            for pos in range(max_pos):
                ...
                if cell_te.sum() == 0:
                    continue
                grid[ri, pos] = float(
                    (preds[cell_te] == y_score[cell_te]).mean())
